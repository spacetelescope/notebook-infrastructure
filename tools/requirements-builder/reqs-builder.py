...
# arrays for convenient use later
lap = np.array(list(all_packages))
drop = np.array([])

# while we have more than two notebooks, continue
while len(dist_matrix>2):
    # calculate distances -- "furthest" notebooks should be dropped
    s = np.sum(dist_matrix, axis=0)
    drop_idx = np.argwhere(max(s)==s)

    # drop value(s) calculated above
    dist_matrix = a.drop_notebook_dist(dist_matrix, drop_idx)
    nb_matrix, dropped_packages = a.drop_notebook_reqs(nb_matrix, drop_idx)

    # current packages we are removing/dropping
    current_drop = lap[dropped_packages]
    
    # catch the case on the first run so we don't run setdiff1d
    ...
    # add the latest dropped packages to the list
